...
A running notebook of dated notes, newest first, that never disappears just because you're not browsing "this month" any more — a note belongs to the day it was written, not to whatever period the ledger happens to be showing. A photo used as a card background gets a **framing step** rather than being centre-cropped and hoped over: the frame is the card's own shape, so what sits inside it while you drag and zoom is exactly what the card shows. The crop is baked in, which makes the stored file smaller rather than carrying pixels nothing will ever display.

Each note can wear its own **skin** — ten gradients (blossom, orchid, iris, dusk, sunset, ocean, forest, ember, gold, ink) or a photo from the gallery, each with a corner sheen and a fine weave over it so a card reads as a surface catching light rather than a flat rectangle of colour. Left on Auto a note takes one by its position in the list, the way it always did.
...
Tiles **reorder by holding one for three seconds** and dragging it where you want it. The hold is deliberately long: these tiles are tapped constantly to mark a habit done and the grid sits inside a scroller, so a quick press-and-hold would turn ordinary use into accidental reordering. A bar fills across the tile while it counts, and any real movement before it completes cancels the pickup as a scroll.
